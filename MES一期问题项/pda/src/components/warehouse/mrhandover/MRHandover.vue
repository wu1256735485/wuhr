<template>
  <div>
    <VHeader :title="'MR物料交接'" @back="handleBack"></VHeader>
    <div class="so-wrapper common-wrapper">
...
import {
  getquerymrpickingdetailbyaggregatecode,
  getmraggregatehandover,
  getsoaggregatehairfeedconfirm
...
export default {
  ...
  methods: {
    ...
    _getList() {
      ...
      getquerymrpickingdetailbyaggregatecode(data).then(res => {
        console.log(res)
        if (res.status === 1) {
          let nwelist = res.result
          nwelist.mrAggregateDetailList.forEach(item => {
    ...
    _materialsConfirm() {
      ...
      this.handleMes()
      getsoaggregatehairfeedconfirm([this.list]).then(res => {
        this.showPopup('myPopup', res.errorMsg)
        if (res.status === 1) {
    ...
    _signConfirm() {
      ...
      getmraggregatehandover([this.list]).then(res => {
        this.showPopup('myPopup', res.errorMsg)
        if (res.status === 1) {
      ...
      getNfc({ nfcNo: code }).then(res => {
        ...
        if (res.result.code === 0) {
          ...
          if (res.result.mes.ORG.split(",").indexOf("llEwFREIU4Q5RpQh") != -1 || res.result.mes.ORG.split(",").indexOf("VVgMAMUIEtt8IR1o") !=-1 || res.result.mes.ORG === "10110") {
            ...
            this.list.mrAggregateDetailList.forEach(item => {
              this.$set(item, 'isDisabled', false)
            })
          } else {
            if (this.list.confirmationStatus !== 2) {
    ...
    getRuterData() {
      this.data_aggregateCode = this.$route.params.datas
    }
  },
  components: {
    List,
  ...
  mounted() {
    if (typeof this.data_aggregateCode === 'undefined') {
      return
    }
    this._getMaterials(this.data_aggregateCode)
  }
}
</script>
